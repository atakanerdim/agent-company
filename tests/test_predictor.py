...
def test_predictions(sirket):
    env = dict(os.environ, SHIFT_DATE="2026-08-20")
    subprocess.run([sys.executable, str(sirket / "kernel/runner.py"), "--agent", "analyst", "--day", "thu"],
                   cwd=sirket, env=env, capture_output=True, text=True)
    env["SHIFT_DATE"] = "2026-08-21"
    p = subprocess.run([sys.executable, str(sirket / "kernel/runner.py"), "--agent", "statistician", "--day", "fri"],
                       cwd=sirket, env=env, capture_output=True, text=True)
    assert p.returncode == 0, p.stderr
    t = json.loads((sirket / "company/data/predictions/statistician/2026-W34.json").read_text())
    assert len(t) == 3
