...
async function office(){
  ...
  for(const p of people){
    let room=rooms.find(r=>r.name===p.room);
    if(!room){ room={name:p.room,people:[]}; rooms.push(room) }
    room.people.push(p);
  }
...
async function predictionDesk(){
  ...
  if(league){
    const rows=Object.entries(league.personas).sort((a,b)=>b[1].points-a[1].points);
    el("league-table").innerHTML=
      ...
      rows.map(([id,k])=>`<tr><td>${esc(shown(id))}</td><td>${k.points}</td>`+
        `<td>${k.exact}</td><td>${k.outcome}</td><td>${k.weeks}</td></tr>`).join("")+"</table>";
    if(window.Chart&&el("league-chart")) new Chart(el("league-chart"),{type:"bar",
      data:{labels:rows.map(x=>shown(x[0])),
            datasets:[{label:"points",data:rows.map(x=>x[1].points),backgroundColor:"#5b8cff"}]},
  ...
  const weeks=[...new Set(sets.map(f=>f.split("/").pop().replace(".json","")))].sort();
  const last=weeks[weeks.length-1];
  let html="";
  for(const f of sets.filter(x=>x.endsWith(last+".json"))){
    const id=f.split("/")[1], data=await j(f); if(!data) continue;
    const p=people[id];
    html+=`<div class="bubble" style="margin:18px 0 8px">${face(id,44)}
  ...
  if(el("archive")) el("archive").innerHTML=
    weeks.map(w=>`<span class="badge" style="margin-right:8px">${esc(w)}</span>`).join("")
